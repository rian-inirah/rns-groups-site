...
// ====== CONTACT SECTION SCROLL TRACKER (WITH SIZE CHECK) ======
        ...
        const observer = new IntersectionObserver(
            entries => {
                entries.forEach(entry => {
                    // Check if we are on a bigger screen (900px or more)
                    if (window.innerWidth >= 900) {
                        if (entry.isIntersecting) {
                            ...
                            // Contact section is out of view (either scrolling up or scrolling far down)
                            const rect = contactSection.getBoundingClientRect();
                            
                            // Check if the Contact section is currently below 60% of the viewport height (meaning we are near the top/Home section)
                            if (rect.top > window.innerHeight * 0.6) {
                                // Highlight Home link
                                document.querySelectorAll(".nav-link").forEach(l => l.classList.remove("is-active"));
                                homeLink.forEach(l => l.classList.add("is-active"));
                            }
                            // Note: If scrolling far down past the contact section, no change is needed, 
                            // as the active link should remain 'Contact' or another link if more sections were present.
                        }
                    }
                });
            },
