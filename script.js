...
if (countrySel) {
    // Populate all countries (no placeholder)
    countrySel.innerHTML = allCountries.map(c => `<option value="${c}">${c}</option>`).join('');
    
    // Set a default selected value (e.g., India) and immediately update the code display
    const defaultCountry = 'India';
    if (countrySel.querySelector(`option[value="${defaultCountry}"]`)) {
...
// --- Real-time Input Filtering (New and Improved) ---
const nameInput = document.getElementById('name');
const phoneInput = document.getElementById('phone');

if (nameInput) {
    nameInput.addEventListener('input', function() {
        // Strip out non-alphabetical characters and non-spaces
...
function validateEmail(email) {
    // 4. Email validation check for existence of '@' symbol
